...
class ParallelParser:
    ...
    def __init__(
        ...
        if executor_type == "process":
            ...
            self.max_workers: int = max_workers if max_workers else 4
        self.logger = setup_logger(os.path.basename(__file__))

    # @staticmethod
    # def split_to_chunks(filename: str, max_workers: int, parser: Parser, fmt_def) -> List[Tuple[int, int]]:
    ...
    #     return chunks
    @staticmethod
    def split_to_chunks(parser: Parser) -> List[Tuple[int, int]]:
        if not parser.data:
        ...
        while start < file_size:
            start = parser.data.find(MSG_HEADER, start)
            if start == -1:
                break

            end = min(start + chunk_size, file_size)

            next_header = parser.data.find(MSG_HEADER, end)
            if next_header == -1:
                end = file_size
            else:
                end = next_header
    ...
    def process_all(self, message_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            with Parser(self.filename) as parser:
                ...
                chunks = ParallelParser.split_to_chunks(parser)
            ...
            if self.executor_type == "thread":
                ...
                build_struct = True
            results = self._run_executor(executor_class, chunks_count, chunks, fmt_def, message_type, build_struct)
