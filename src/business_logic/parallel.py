...
class ParallelParser:
    ...
    def __init__(
        self, file_path: str, max_workers: Optional[int] = None
    ):
    ...
    def process_all(self, message_type: Optional[str] = None, executor_type: Literal["process", "thread"] = "process") -> List[Dict[str, Any]]:
        ...
        try:
            with Parser(self.file_path) as parser:
                ...
                } if executor_type == "process" else parser.format_definitions
    ...
    def _run_executor(
        ...
        with executor_class(max_workers=self.max_workers) as executor:
            chunk_results = executor.map(
                ...
                repeat(self.file_path, chunks_count),
                chunks,
                repeat(fmt_definitions, chunks_count),
    ...
    def _process_chunk(
        ...
        try:
            ...
            with Parser(filename) as parser:
                parser.format_definitions = format_definitions
    ...
    def _split_to_chunks(parser: Parser, max_workers: int) -> List[Tuple[int, int]]:
        ...
        try:
            ...
            data = parser.data
            fmt_definitions = parser.format_definitions
            size = len(data)
            ...
            while True:
                ...
                if is_valid_message_header(data, pos, fmt_definitions):
                    break
                pos += header_len
            ...
            while pos < size:
                ...
                while next_pos != -1 and not is_valid_message_header(data, next_pos, fmt_definitions):
